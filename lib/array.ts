...
export function newArray(deep: any) {
  ...
  _Array._contain.sortedView = new deep.Method(function(this: any, compareFn?: (a: any, b: any) => number) {
    const self = new deep(this._source);
    ...
    // Create result array with sorted data
    const sortedArray = new deep.Array(sortedData);
    
    // Store the sort function and source array in the sorted array's state for reactive updates
    sortedArray._state._sortFn = compareFn;
    sortedArray._state._sourceArray = self;
    
    // Set up reactive tracking by assigning trackable function to _state._onTracker
    sortedArray._state._onTracker = deep._contain.Array._contain.sortedView._contain.trackable.data;
    
    // Create tracker to link source array to sorted array
    const tracker = self.track(sortedArray);
    sortedArray._state._sourceTracker = tracker;
    
    return sortedArray;
  });

  // Add trackable to sortedView method for point-wise updates
  _Array._contain.sortedView._contain.trackable = new deep.Trackable(function(this: any, event: any, ...args: any[]) {
    ...
    const findInsertionPosition = (arr: any[], element: any, compareFn?: (a: any, b: any) => number): number => {
      let left = 0;
    ...
    if (event.is(deep.events.dataAdd)) {
      ...
      for (const addedElement of args) {
        ...
        const insertPos = findInsertionPosition(sortedArray._data, originalValue, sortFn);
        
        // Insert element at correct position
        sortedArray._data.splice(insertPos, 0, originalValue);
        
        // Emit events
        sortedArray.emit(deep.events.dataAdd, addedElement);
      ...
      for (const deletedElement of args) {
        ...
        const deleteIndex = sortedArray._data.indexOf(originalValue);
        if (deleteIndex > -1) {
          sortedArray._data.splice(deleteIndex, 1);
          
          // Emit events
          sortedArray.emit(deep.events.dataDelete, deletedElement);
          sortedArray.emit(deep.events.dataChanged);
        }
      }
    } else if (event.is(deep.events.dataSet)) {
      ...
      const insertPos = findInsertionPosition(sortedArray._data, newValue, sortFn);
      sortedArray._data.splice(insertPos, 0, newValue);
      
      // Emit events
      sortedArray.emit(deep.events.dataChanged);
    } else if (event.is(deep.events.dataPush)) {
      // Handle push operations (similar to dataAdd)
      for (const pushedElement of args) {
        const originalValue = pushedElement._symbol;
        ...
        // Emit events
        sortedArray.emit(deep.events.dataPush, pushedElement);
        sortedArray.emit(deep.events.dataChanged);
      }
    } else {
      // Fallback for other events - do full recalculation
      const sortedData = [...sourceArray._data];
      if (sortFn) {
