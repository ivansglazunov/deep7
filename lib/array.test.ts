...
describe('Array', () => {
  ...
  it('should create reactive sorted view with default compareFn', () => {
    const deep = newDeep();
    const arr = new deep.Array([3, 1, 4, 1, 5]);
    
    const sortedArr = arr.sortedView();
    
    expect(sortedArr.type.is(deep.Array)).toBe(true);
    expect(sortedArr._data).toEqual([1, 1, 3, 4, 5]);
    expect(arr._data).toEqual([3, 1, 4, 1, 5]); // Original array should be unchanged
  });

  it('should create reactive sorted view with custom compareFn', () => {
    const deep = newDeep();
    const arr = new deep.Array([3, 1, 4, 1, 5]);
    
    const sortedArr = arr.sortedView((a: number, b: number) => b - a); // Descending order
    
    expect(sortedArr._data).toEqual([5, 4, 3, 1, 1]);
    expect(arr._data).toEqual([3, 1, 4, 1, 5]); // Original array should be unchanged
  });

  it('should make array.sortedView() reactive using tracking system', () => {
    const deep = newDeep();
    
    const sourceArray = new deep.Array([3, 1, 4]);
    const sortedArray = sourceArray.sortedView();
    
    // Verify initial sorting
    expect(sortedArray._data).toEqual([1, 3, 4]);
    
    // Test reactivity by pushing elements (not add to avoid deduplication)
    sourceArray.push(2);
    ...
    // Test another push operation
    sourceArray.push(2.5);
    expect(sortedArray._data).toEqual([0, 1, 2, 2.5, 4, 5]);
    
    // Test set operation
    sourceArray.set(0, 10); // Change first element (3) to 10
    expect(sortedArray._data).toEqual([0, 1, 2, 2.5, 4, 5, 10]);
  });

  it('should handle reactive sorted view with custom compareFn', () => {
    const deep = newDeep();
    
    const sourceArray = new deep.Array([3, 1, 4]);
    const descendingSorted = sourceArray.sortedView((a: number, b: number) => b - a);
    
    // Verify initial sorting (descending)
    expect(descendingSorted._data).toEqual([4, 3, 1]);
    
    // Test reactivity with custom sort
    sourceArray.add(2);
    expect(descendingSorted._data).toEqual([4, 3, 2, 1]);
    
    sourceArray.add(5);
    expect(descendingSorted._data).toEqual([5, 4, 3, 2, 1]);
    
    // Test removing
    sourceArray.delete(3);
    expect(descendingSorted._data).toEqual([5, 4, 2, 1]);
  });

  it('should verify that Array.sortedView is trackable', () => {
    const deep = newDeep();
    
    // Test that Array.sortedView is trackable
    expect(deep.Array.sortedView.isTrackable).toBe(true);
    
    // Test that Array.sortedView has trackable in context
    expect(deep.Array.sortedView._contain.trackable).toBeDefined();
    expect(deep.Array.sortedView._contain.trackable.type.is(deep.Trackable)).toBe(true);
  });

  it('should handle complex objects with custom sort function', () => {
    const deep = newDeep();
    ...
    const sortedByAge = sourceArray.sortedView((a: any, b: any) => a.age - b.age);
    
    expect(sortedByAge._data).toEqual([
      { name: 'Alice', age: 25 },
      { name: 'Bob', age: 30 },
      { name: 'Charlie', age: 35 }
    ]);
