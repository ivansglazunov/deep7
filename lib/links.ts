...
function emitReferrerChangeEvents(deep: any, changedInstanceId: string, __isStorageEvent?: string, visited = new Set()) {
  ...
  for (const { relation, eventName, reverseLinkProp } of relationsToNotify) {
    ...
    for (const referrerId of referrers) {
      ...
      if (referrerInstance[reverseLinkProp] === changedInstanceId) {
        const payload = createLinkEventPayload(deep, referrerId, eventName, undefined, __isStorageEvent);
...
export function newType(deep: any) {
  const Type = new deep.Field(function(this: any, key: any, value: any) {
    ...
    if (this._reason == deep.reasons.getter._id) {
      ...
      new deep(sourceId).emit(deep.events.typeSetted._id, createLinkEventPayload(deep, sourceId, deep.events.typeSetted._id, { before: oldTargetId }, __isStorageEvent));
...
export function newFrom(deep: any) {
  const From = new deep.Field(function(this: any, key: any, value: any) {
    ...
    if (this._reason == deep.reasons.getter._id) {
      ...
      new deep(sourceId).emit(deep.events.fromSetted._id, createLinkEventPayload(deep, sourceId, deep.events.fromSetted._id, { before: oldTargetId }, __isStorageEvent));
      if (oldTargetId !== newTargetId) {
        if (oldTargetId) {
...
export function newTo(deep: any) {
  const To = new deep.Field(function(this: any, key: any, value: any) {
    ...
    if (this._reason == deep.reasons.getter._id) {
      ...
      new deep(sourceId).emit(deep.events.toSetted._id, createLinkEventPayload(deep, sourceId, deep.events.toSetted._id, { before: oldTargetId }, __isStorageEvent));
      if (oldTargetId !== newTargetId) {
        if (oldTargetId) {
...
export function newValue(deep: any) {
  const Value = new deep.Field(function(this: any, key: any, value: any) {
    ...
    if (this._reason == deep.reasons.getter._id) {
      ...
      new deep(sourceId).emit(deep.events.valueSetted._id, createLinkEventPayload(deep, sourceId, deep.events.valueSetted._id, { before: oldTargetId }, __isStorageEvent));
      if (oldTargetId !== newTargetId) {
        if (oldTargetId) {
