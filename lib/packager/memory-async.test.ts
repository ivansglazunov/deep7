...
it('packager:memory-async', async () => {
  // <deep1>
  // some who make tools
  const { deep1, storage1_tools1, storage1_tools2, storage1_tools3 } = await (async () => {
    ...
    const storage1_tools1 = new deep1.Storage.MemoryAsync({
      package: tools1,
    });
    ...
    const storage1_tools2 = new deep1.Storage.MemoryAsync({
      package: tools2,
    });
    await storage1_tools2.mount(); // any initial errors will be throw here and unmount storage

    const storage1_tools3 = new deep1.Storage.MemoryAsync({
      package: tools3,
    });
  ...
  const { deep2, storage2_tools1, storage2_tools2, storage2_tools3, storage2_personal1, a2, b2, c2 } = await (async () => {
    ...
    const storage2_tools1 = new deep2.Storage.MemoryAsync({
    ...
    const storage2_tools2 = new deep2.Storage.MemoryAsync({
    ...
    const storage2_tools3 = new deep2.Storage.MemoryAsync({
    ...
    const storage2_personal1 = new deep2.Storage.MemoryAsync({
      query: deep2.query({
        _or: [
          { type: A2 },
          { type: B2 },
  ...
  const { deep3, storage3_personal1 } = await (async () => {
    ...
    const storage3_personal1 = new deep3.Storage.MemoryAsync({
  ...
  await (async () => {
    ...
    const storage3_tools1 = new deep3.Storage.MemoryAsync({
      memory: storage1_tools1.state._memory,
    });
    await storage3_tools1.mount(); // and await mounting

    const storage3_tools2 = new deep3.Storage.MemoryAsync({
      memory: storage1_tools2.state._memory,
      subscribe: false,
    });
    await storage3_tools2.mount();

    const storage3_tools3 = new deep3.Storage.MemoryAsync({
      memory: storage1_tools3.state._memory,
    });
