...
export class _FsJsonAsync extends _Memory {
  ...
  constructor(path: string) {
    super();
    this._path = path;
    this.debug = Debug(`packager:_fs-json-async:${path}`);
  }
  async save(object: SerializedPackage): Promise<void> {
    this.debug('save', object);
    await fs.promises.writeFile(this._path, jsan.stringify(object));
    await this.notify();
  }
  _loaded?: any;
  async load(): Promise<SerializedPackage> {
    this.debug('load');
    try {
      this._loaded = jsan.parse(await fs.promises.readFile(this._path, 'utf8'));
    } catch (e) {
      this._loaded = { data: [] };
    }
    return this._loaded;
