...
import { _unwatch } from './github-gists';
...
(!!process.env.GITHUB_TOKEN ? it : it.skip)('packager:fs-json-async', async () => {
  ...
  const gist = await octokit.rest.gists.create({
    description: `deep7:packager:github-gist.test.${Date.now()}`,
    files: {
      'tools1.json': { content: JSON.stringify({ data: [] }), },
      'tools2.json': { content: JSON.stringify({ data: [] }), },
      'tools3.json': { content: JSON.stringify({ data: [] }), },
      'personal1.json': { content: JSON.stringify({ data: [] }), },
    },
    public: false,
  });
  if (gist.status != 201) throw new Error('Failed to create gist');
  const gist_id = gist.data.id;
  ...
  const { deep1, storage1_tools1, storage1_tools2, storage1_tools3 } = await (async () => {
    ...
    const storage1_tools1 = new deep1.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools1.json',
      package: tools1,
    });
    ...
    const storage1_tools2 = new deep1.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools2.json',
      package: tools2,
    });
    await storage1_tools2.mount(); // any initial errors will be throw here and unmount storage

    const storage1_tools3 = new deep1.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools3.json',
      package: tools3,
    });
  ...
  const { deep2, storage2_tools1, storage2_tools2, storage2_tools3, storage2_personal1, a2, b2, c2 } = await (async () => {
    ...
    const storage2_tools1 = new deep2.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools1.json',
    });
    ...
    const storage2_tools2 = new deep2.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools2.json',
      subscribe: false,
    ...
    const storage2_tools3 = new deep2.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools3.json',
    });
    ...
    const storage2_personal1 = new deep2.Storage.GithubGist({
      octokit,
      gist_id,
  ...
  const { deep3, storage3_personal1 } = await (async () => {
    ...
    const storage3_personal1 = new deep3.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'personal1.json',
    });
    ...
    await (async () => {
      ...
      expect(storage3_personal1.errors).toEqual([
        ...
        `Failed to deserialize (${a2._id}), .value (${a2._value}) not founded.`,
        `Failed to deserialize (${b2._id}), .type (tools2/B) not founded.`,
        `Failed to deserialize (${b2._id}), .value (${b2._value}) not founded.`,
        `Failed to deserialize (${c2._id}), .type (tools3/C) not founded.`,
        `Failed to deserialize (${c2._id}), .value (${c2._value}) not founded.`,
      ]);
  ...
  await (async () => {
    ...
    await storage2_personal1.update(); // GithubGist not support subscribe, so we need to update manually
    await storage3_personal1.update(); // GithubGist not support subscribe, so we need to update manually
  ...
  await (async () => {
    ...
    const storage3_tools1 = new deep3.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools1.json',
    });
    ...
    const storage3_tools2 = new deep3.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools2.json',
    });
    ...
    const storage3_tools3 = new deep3.Storage.GithubGist({
      octokit,
      gist_id,
      filename: 'tools3.json',
    });
