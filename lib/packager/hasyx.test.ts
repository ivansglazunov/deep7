...
(hasHasura ? it : it.skip)('packager:hasyx', async () => {
  ...
  const [_deep] = await hasyx.select({
    table: 'deep_links',
    where: { id: { _eq: DEEP_ID } },
  });
  if (!_deep) await hasyx.insert({
    table: 'deep_links',
    object: {
      id: DEEP_ID,
      _deep: DEEP_ID,
  ...
  const { deep2, storage2_tools1, storage2_tools2, storage2_tools3, storage2_personal1, a2, b2, c2 } = await (async () => {
    ...
    await (async () => {
      ...
      expect(_.sortBy((await storage2_personal1.load()).data.map(l => ({ ...l, _created_at: 0, _updated_at: 0 })), 'id')).toEqual(_.sortBy([
        storage2_personal1.serialize(a2),
        storage2_personal1.serialize(b2),
        storage2_personal1.serialize(c2),
      ].map(l => ({ ...l, _created_at: 0, _updated_at: 0 })), 'id'));
    })();
  ...
  await (async () => {
    ...
    await (async () => {
      ...
      expect(storage3_personal1.errors).toContain(`Failed to deserialize (${b2._id}), .type (tools2/B) not founded.`);
      expect(storage3_personal1.errors).toContain(`Failed to deserialize (${c2._id}), .type (tools3/C) not founded.`);
      expect(storage3_personal1.errors.length).toBe(3);
