...
export class _MemorySync extends _Memory {
  debug = Debug('packager:memory-sync');
...
export function newPackagerMemorySync(deep: Deep) {
  const debug = Debug(`packager:memory-sync:${deep._id}`);

  const MemorySync = deep.Storage.MemorySync = new deep.Storage();
    ...
    if (lifestate == deep.Constructed) {
      ...
      storage.state._resubscribe = async() => {
        ...
        if (storage.state._subscribe) storage.state._memory_unsubscribe = await storage.memory.subscribe(async (object) => {
          ...
          await storage.deserializePackage(object);
          delete storage.errors;
          await storage.patch(object);
        }); 
        return preloaded;
      };
